...
export async function POST(request: Request) {
  ...
  try {
    ...
    const postcodeParts = data.housePostcode?.toString().split(/\s+/); // split the postcode based on the white space, for example SE17 1PE into SE17 and 1PE
    ...
    const [pricesPaidLvl3] = await connection.execute<RowDataPacket[]>(getPricesLvl3Query); // execute the query and retrieve the results
    ...
    if (
      ...
      const [pricesPaidLvl2] = await connection.execute<RowDataPacket[]>(getPricesLvl2Query); // execute the query and retrieve the results
      ...
      if (
        ...
        const [pricesPaidLvl1] = await connection.execute<RowDataPacket[]>(getPricesLvl1Query); // execute the query and retrieve the results
    ...
    const [buildPrice] = await connection.execute<RowDataPacket[]>(getBuildPriceQuery); // execute the query and retrieve the results
    ...
    // get the rent value --> Note: this need to change to accommodate future data
    const getSocialRentQuery = `SELECT earningsPerWeek FROM fairhold.socialRent JOIN fairhold.itl3 ON fairhold.itl3.itl3 LIKE CONCAT(fairhold.socialRent.itl3,'%') WHERE postcode ='${data.housePostcode}'`; // create the sql query
    const [Socialrent] = await connection.execute<RowDataPacket[]>(getSocialRentQuery); // execute the query and retrieve the results
    ...
    return NextResponse.json({
      ...
      houseBedrooms: data.houseBedrooms ? parseFloat(data.houseBedrooms.toString()) : null,
