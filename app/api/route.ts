  ...
  try {
    ...
    if (data.housePostcode != null) {
      ...
      if (postcode.valid) {
        ...
        const postdoceSearch3 = postcodeSector + "%"; // add the % for SQL query
        ...
        if (
          ...
          const postdoceSearch2 = postcodeDistrict + "%"; // add the % for SQL query
          ...
          const [pricesPaidDistrict] = await connection.execute<
            RowDataPacket[]
          >(getPricesLvl2Query); // execute the query and retrieve the results
          const numberOfpricesPaidDistrict =
            Object.keys(pricesPaidDistrict).length; // extract the number of entries
          ...
          if (
            ...
            const postdoceSearch1 = postcodeArea + "%"; // add the % for SQL query
